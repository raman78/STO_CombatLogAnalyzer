...
### Fixes
- comparing combats now opens on the same player in each of them. Each combat used to be opened on its own highest-DPS player, and in a team that is rarely the same person, so the differences compared one player against another instead of one player's runs — which read as random. If a combat did not include that player it keeps its own top one, and a warning above the table says the numbers are from different people
